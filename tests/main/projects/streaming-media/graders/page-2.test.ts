...
import { page2Grader } from '../../../../../src/main/projects/streaming-media/graders/page-2';
...
test('Grader passes good assignment', async () => {
  ...
  const result = await page2Grader.run(new URL(goodStreamingMediaBaseUrl));
...
test('Grader fails bad assignment', async () => {
  ...
  const result = await page2Grader.run(new URL(badStreamingMediaBaseUrl));
...
test('Grader fails if the HTML body is empty', async () => {
  const expected = { status: 'fail', message: expect.any(String) } satisfies GraderResult;
  const result = await page2Grader.run(new URL(emptyStreamingMediaBaseUrl));
...
test('Grader fails a non 200 response', async () => {
  const expected = { status: 'fail', message: expect.any(String) } satisfies GraderResult;
  const result = await page2Grader.run(new URL(httpErrorStreamingMediaBaseUrl));
...
test('Grader errors on network error', async () => {
  ...
  const result = await page2Grader.run(new URL(networkErrorStreamingMediaBaseUrl));
