...
test('Grader passes good assignment', async () => {
  ...
  const result = await dankmemesImageGrader.run(goodDankmemesImageUrl);
...
test('Grader fails bad assignment', async () => {
  ...
  const result = await dankmemesImageGrader.run(badDankmemesImageUrl);
...
test('Grader fails if the image body is empty', async () => {
  const expected = { status: 'fail', message: expect.any(String) } satisfies GraderResult;
  const result = await dankmemesImageGrader.run(emptyDankmemesImageUrl);
...
test("Grader fails if the content type is not 'image/png'", async () => {
  const expected = { status: 'fail', message: expect.any(String) } satisfies GraderResult;
  const result = await dankmemesImageGrader.run(badContentTypeDankmemesImageUrl);
...
test('Grader fails a non 200 response', async () => {
  const expected = { status: 'fail', message: expect.any(String) } satisfies GraderResult;
  const result = await dankmemesImageGrader.run(httpErrorDankmemesImageUrl);
...
test('Grader errors on network error', async () => {
  ...
  const result = await dankmemesImageGrader.run(networkErrorDankmemesImageUrl);
